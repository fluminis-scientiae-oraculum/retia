...
impl crate::fts::tokenizer::Tokenizer for CangJieTokenizer {
    ...
    fn token_stream<'a>(&self, text: &'a str) -> BoxTokenStream<'a> {
        let result: Vec<&str> = match self.option {
            TokenizerOption::All => self.worker.cut_all(text).into_iter().map(|t| t.word).collect(),
            TokenizerOption::Default { hmm: use_hmm } => {
                self.worker.cut(text, use_hmm).into_iter().map(|t| t.word).collect()
            }
            TokenizerOption::ForSearch { hmm: use_hmm } => {
                self.worker.cut_for_search(text, use_hmm).into_iter().map(|t| t.word).collect()
            }
