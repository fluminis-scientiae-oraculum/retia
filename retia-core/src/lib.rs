...
//! This crate is **retia**, a Rust-only fork of [CozoDB](https://github.com/cozodb/cozo).
//! It may be used to embed the database engine in your application.
//!
//! This doc describes the Rust API. To learn the query language (RetiaScript), see
//! the upstream documentation, which still applies to retia:
//!
...
impl DbInstance {
    ...
    pub fn new(engine: &str, path: impl AsRef<Path>, options: &str) -> Result<Self> {
        ...
        Ok(match engine {
            "mem" => Self::Mem(new_retia_mem()?),
            #[cfg(feature = "storage-sqlite")]
            "sqlite" => Self::Sqlite(new_retia_sqlite(path)?),
            #[cfg(feature = "storage-rocksdb")]
    ...
    /// Run the RetiaScript passed in. The `params` argument is a map of parameters formatted as JSON.
    /// See [crate::Db::run_script].
