...
impl Ord for UuidWrapper {
    // Compare by raw bytes. This makes v7 UUIDs (timestamp-prefixed) sort
    // chronologically, which is the whole point of v7. v1 sorts by its high
    // gregorian-epoch bits (byte 6-7 first), which is *not* clock-time order
    // — chronological v1 was a CozoDB quirk that we drop in favor of v7.
    // v4 sorts randomly. This Ord must mirror the memcmp encoding in
    // `data/memcmp.rs` so on-disk and in-memory orderings agree.
