...
fn test_uuid() {
    ...
    let default = op_rand_uuid(&[]).unwrap();
    assert!(op_is_uuid(&[v4]).unwrap().get_bool().unwrap());
    assert!(op_is_uuid(&[v7.clone()]).unwrap().get_bool().unwrap());
    assert!(op_is_uuid(&[default.clone()]).unwrap().get_bool().unwrap());
    assert!(op_uuid_timestamp(&[v1]).unwrap().get_float().is_some());
    assert!(op_uuid_timestamp(&[v7]).unwrap().get_float().is_some());
    assert!(op_uuid_timestamp(&[default]).unwrap().get_float().is_some());
    assert!(op_to_uuid(&[DataValue::from("")]).is_err());
    assert!(op_to_uuid(&[DataValue::from("f3b4958c-52a1-11e7-802a-010203040506")]).is_ok());
}

#[test]
fn test_uuid_v7_sorts_chronologically() {
    let mut prev = op_rand_uuid_v7(&[]).unwrap();
    for _ in 0..50 {
        // Force at least 1 ms between generations so the time prefix advances.
        std::thread::sleep(std::time::Duration::from_millis(2));
        let next = op_rand_uuid_v7(&[]).unwrap();
        assert!(next > prev, "v7 UUIDs must sort chronologically");
        prev = next;
    }
}
