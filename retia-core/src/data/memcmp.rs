    ...
    fn encode_datavalue(&mut self, v: &DataValue) {
        match v {
            ...
            DataValue::Uuid(u) => {
                ...
                // Natural byte order so v7 UUIDs (timestamp-prefixed) sort
                // chronologically. Must match the `UuidWrapper::Ord` impl in
                // `data/value.rs`.
                self.write_all(u.0.as_bytes()).unwrap();
            }
            DataValue::Regex(rx) => {
    ...
    pub(crate) fn decode_from_key(bs: &[u8]) -> (Self, &[u8]) {
        ...
        match *tag {
            ...
            UUID_TAG => {
                ...
                bytes.copy_from_slice(uuid_data);
