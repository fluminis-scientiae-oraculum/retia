...
use retia_rocks::{DbBuilder, DbIter, RocksDb, Tx};
...
pub fn new_retia_rocksdb(path: impl AsRef<Path>) -> Result<Db<RocksDbStorage>> {
...
mod tests {
    ...
    fn setup_test_db() -> Result<(TempDir, Db<RocksDbStorage>)> {
        ...
        let db = new_retia_rocksdb(temp_dir.path())?;
