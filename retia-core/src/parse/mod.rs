 ...
 */
//! AST for Retia scripts, for generating Retia scripts programmatically.
//!
...
#[grammar = "retiascript.pest"]
pub(crate) struct RetiaScriptParser;

pub(crate) type Pair<'a> = pest::iterators::Pair<'a, Rule>;
pub(crate) type Pairs<'a> = pest::iterators::Pairs<'a, Rule>;

/// This represents a full Retia script, as you'd pass to `run_script`.
#[derive(Debug)]
pub enum RetiaScript {
    #[allow(missing_docs)]
    Single(InputProgram),
    #[allow(missing_docs)]
...
pub(crate) fn parse_type(src: &str) -> Result<NullableColType> {
    let parsed = RetiaScriptParser::parse(Rule::col_type_with_term, src)
        .into_diagnostic()?
        .next()
        .unwrap();
...
pub(crate) fn parse_expressions(
    ...
    let parsed = RetiaScriptParser::parse(Rule::expression_script, src)
        .map_err(|err| {
...
/// This parses a text script into the AST used by Retia.
///
...
pub fn parse_script(
    ...
    let parsed = RetiaScriptParser::parse(Rule::script, src)
        .map_err(|err| {
            let span = match err.location {
                InputLocation::Pos(p) => SourceSpan(p, 0),
                InputLocation::Span((start, end)) => SourceSpan(start, end - start),
