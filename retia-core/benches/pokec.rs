...
lazy_static! {
    static ref ITERATIONS: usize = {
        let size = env::var("RETIA_BENCH_ITERATIONS").unwrap_or("100".to_string());
        size.parse::<usize>().unwrap()
    };
    static ref SIZES: (usize, usize) = {
        let size = env::var("RETIA_BENCH_POKEC_SIZE").unwrap_or("medium".to_string());
        match &size as &str {
            "small" => (10000, 121716),
    ...
    static ref TEST_DB: DbInstance = {
        let data_dir = PathBuf::from(env::var("RETIA_BENCH_POKEC_DIR").unwrap());
        let db_kind = env::var("RETIA_TEST_DB_ENGINE").unwrap_or("mem".to_string());
        let mut db_path = data_dir.clone();
        let data_size = env::var("RETIA_BENCH_POKEC_SIZE").unwrap_or("medium".to_string());
        let batch_size = env::var("RETIA_BENCH_POKEC_BATCH")
            .unwrap()
            .parse::<usize>()
            .unwrap();
...
fn mixed(_: &mut Bencher) {
    ...
    let mixed_pct = env::var("RETIA_BENCH_POKEC_MIX_PCT").unwrap_or("0.3".to_string());
