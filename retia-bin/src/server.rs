...
use retia::{DataValue, DbInstance, format_error_as_json, MultiTransaction, NamedRows, ScriptMutability, SimpleFixedRule};
...
pub(crate) struct ServerArgs {
    ...
    #[clap(short, long, default_value_t = String::from("retia.db"))]
    path: String,
...
impl AsyncAuthorizeRequest<Body> for MyAuth
{
    ...
    fn authorize(&mut self, mut request: Request<Body>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let mutability = match request.headers().get("x-retia-auth") {
...
pub(crate) async fn server_main(args: ServerArgs) {
    ...
    let conf_path = if skip_auth {
        ...
        format!("{}.{}.retia_auth", args.path, args.engine)
    };
    ...
    let cors = CorsLayer::new()
        ...
        .allow_origin(Any)
        .allow_headers([header::CONTENT_TYPE, HeaderName::from_static("x-retia-auth")]);
    ...
    info!(
        "Starting Retia ({}-backed) API at http://{}",
        args.engine, addr
    );
